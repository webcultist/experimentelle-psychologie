...
const LP = (() => {
  ...
  function injectStyles() {
    ...
    s.textContent = `
      ...
      /* Settings panel */
      #lp-settings-panel{position:fixed;bottom:60px;left:12px;z-index:9001;background:var(--bg-card,#fff);border:1px solid var(--border,#ddd);border-radius:12px;padding:16px;width:240px;box-shadow:0 4px 20px rgba(0,0,0,.15);transform:translateX(-280px);transition:transform .3s;font-size:.9rem}
  ...
  function injectUI() {
    ...
    panicBtn.innerHTML = `<button class="lp-fab" id="lp-panic-btn" title="Panik-Modus: Zeig nur einen Chunk" style="background:#e74c3c;color:#fff;bottom:auto;right:auto">😵</button>`;
    ...
    document.getElementById('lp-panic-btn').addEventListener('click', () => {
      if (panic.active) {
        ...
        nextBtn.id = 'lp-panic-next'; nextBtn.className = 'lp-fab';
        nextBtn.style.cssText = 'background:#2c6e49;color:#fff;bottom:auto;right:auto;border-radius:8px;width:auto;padding:0 12px;font-size:.85rem';
        nextBtn.textContent = 'Weiter →';
        nextBtn.addEventListener('click', () => panic.next());
        const exitBtn = document.createElement('button');
        exitBtn.className = 'lp-fab'; exitBtn.style.cssText = 'background:#555;color:#fff;bottom:auto;right:auto;border-radius:8px;width:auto;padding:0 12px;font-size:.85rem';
        exitBtn.textContent = 'Alles zeigen';
        exitBtn.addEventListener('click', () => { panic.exit(); panicBtn.querySelectorAll('.lp-fab:not(#lp-panic-btn)').forEach(b => b.remove()); });
